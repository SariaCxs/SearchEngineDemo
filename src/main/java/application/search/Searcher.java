...
public class Searcher {
    ...
    public Set<Integer> PagesMatchedInTitle(List<String> querytokens){
        ...
        for(String token:querytokens){
            if (!token.contains(" ")){
                ...
                int wordID=indexerController.getWordIdByWord(token);
                ...
                for(int i=0;i<words.size();i++){
                    // change token to wordID
                    int wordID=indexerController.getWordIdByWord(words.get(i));
    ...
    public HashSet<Integer> findIntersection(List<String> words){
        ...
        for(String word:words){
            int wordId = indexerController.getWordIdByWord(word);
...
//        double minMatchingScore = Double.MAX_VALUE;
        ...
        for (Map.Entry<Integer, Map<String, Double>> entry : documentVectors.entrySet()){
            ...
            double pageRankValue = indexerController.getPageRankValue(pageId) * 10;
            double score = 0.3 * cosineSimilarity + 0.7 * pageRankValue;
